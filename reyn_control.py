...
class Reynolds_Solver: 
    ...
    def fd_solve(self, N):
        ...
        mat = fd.make_mat(ex)
        ps = np_solve(mat, rhs)
        pressure  = Pressure(self.Example.height, ps)
        return pressure
    ...
    def solve_and_plot(self, N):
        ...
        p_labels = ["$x$", "y","Pressure $p(x)$"]
        # graphics.plot_2D(pwl_pressure.ps_1D, ex.xs, p_title, p_labels, color='r')
        graphics.plot_contour_mesh(pwl_pressure.ps_2D, ex.xs, ex.ys, p_title, p_labels, log_cmap=False)
        ...
        vmax = min(np.max(vel.vx),10)
        graphics.plot_stream_heat(vel.vx, vel.vy, ex.xs, ex.ys, uv_mag, v_title, v_ax_labels, vmin=0, vmax=vmax)
