...
class Velocity:
    ...
    def get_flux(self, vx):
        ...
        q = np.average(qs)
        # q = stats.mode(qs, keepdims=False)[0]
        # print(q)
        return q
...
class AdjReynVelocity(Velocity):
    ...
    def __init__(self, height, adj_ps):
        ...
        # inc = self.make_inc(height, vx, vy)
    ...
    def make_adj_velocity(self, height, ps):
        ...
        for i in range(height.Nx):
            ...
            # f1x = -1/(2*height.visc) * (px_hs[i]*hxs[i] + pxx_hs[i]*h) + height.U/(h**2)
    ...
    def make_inc(self, height, u, v):
        ...
        pxx_y = np.zeros((height.Ny, height.Nx))
        dx = height.dx
        ...
        #     for j in range(height.Ny):
        #         if j == 0:
        #             pxxy = domain.right_first(dx, pxx[0 : j+3, i])
                
        #         elif height.ys[j] == h or height.ys[j] + dx > h:
        #             pxxy = domain.left_first(dx, pxx[j-2 : j+1, i])
        #         elif height.ys[j] < h:
        #             pxxy = domain.center_first(dx, pxx[j-1 : j+2, i])
        #         else:
        #             pxxy = 0
        #         pxx_y[j,i] = pxxy * (height.ys[j]**3)/(6*height.visc)
