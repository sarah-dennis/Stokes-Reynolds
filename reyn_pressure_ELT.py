...
class ELT_Pressure(Pressure):
    def __init__(self, height, BC, reyn_pressure, TG):
             
        ps_1D = reyn_pressure.ps_1D
        ps_2D, reyn_derivs, sigma_derivs = self.make_ELT_ps(height, BC, ps_1D, TG)
    ...
    def make_ELT_ps(self, height, BC, reyn_ps, TG=False):
       ...
       #---------------------------------------------------------------------------
        ...
        if TG:
            ...
            sigmas, sigma_xs, sigma_2xs = self.make_sigmas(height,BC, pxs,p2xs,p3xs,p4xs)
    ...
    def adj_rhs(self, height, BC, pxs, p2xs, p3xs, p4xs):
    ...
    def make_sigmas(self, height, BC, pxs, p2xs, p3xs, p4xs):
        ...
        if isinstance(BC, bc.Fixed): # match reyn dP
            ...
            rhs = self.adj_rhs(height, BC, pxs, p2xs, p3xs, p4xs) # with different rhs
...
class VA_ELT_Pressure(ELT_Pressure):
    def __init__(self, height, BC, reyn_pressure):

        super().__init__( height, BC, reyn_pressure, TG = False)


class TG_ELT_Pressure(ELT_Pressure):
    def __init__(self, height, BC, reyn_pressure):


        super().__init__( height, BC, reyn_pressure, TG = True)